...
/**
 * Sends a predefined quick prompt through the same path.
 */
    ...
    <div class="shrink-0 border-t border-[color:var(--color-line-2)] bg-surface-base px-[2.4rem] pb-[1.6rem] pt-[1.2rem]">
        <div class="mx-auto w-full max-w-[84rem]">
            <div class="mb-[1rem] flex flex-wrap items-center gap-[0.6rem]">
                <span class="pr-[0.4rem] text-eyebrow-sm text-text-tertiary">Подсказки ↴</span>
                <button
                    v-for="prompt in quickPrompts"
                    :key="prompt.key"
                    type="button"
                    class="rounded-full border border-[color:var(--color-teal-soft)] bg-brand-ghost px-[1rem] py-[0.5rem] text-[1.2rem] font-medium text-brand-deep transition hover:bg-brand-soft disabled:opacity-60"
                    :disabled="disabled"
            ...
            <div class="flex items-end gap-[1rem] rounded-[1.2rem] border border-[color:var(--color-line-2)] bg-white px-[1.2rem] py-[1rem] shadow-[0_2px_10px_rgb(10_31_31_/_0.04)]">
                <textarea
                    ref="textareaRef"
                    v-model="message"
                ...
                <button
                    ...
                    @click="handleSend"
                >
                    <template v-if="isSendPending">Отправка…</template>
                    <template v-else>
                        Отправить
                        <svg
                            width="10"
            ...
            <div class="mt-[0.6rem] flex items-center justify-between text-[1.05rem] text-text-tertiary">
                ...
                <span class="font-mono">MedMentor-RAG v1.2</span>
            </div>
        </div>
    </div>
</template>

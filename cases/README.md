...
- `correctDiagnosis` is documented as an exact match to one `diagnosisOptions` value, but that cross-field rule is not
  enforced in the schema yet.
- `difficulty` and `patientSex` are intentionally left as open strings for now so authoring can evolve without a schema
  migration.
...
- `COMPLETED`: score and summary are persisted; the session becomes part of history and no longer blocks starting a new
  one.
